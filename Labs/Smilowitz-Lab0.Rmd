...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    ...
    **Over time, the number of girls baptized trends upwards, although there was a significant decline between roughly 1640 and 1660.  There are also a number of other drops in the number of girls baptized over the time range.**
...
the future.  
    ...
    ```
    **The plot for the number of boys baptized is very similar to the plot for the number of girls baptized, with an overall increase but significant decrease between 1640 and 1660.**
...
-   What years are included in this data set? What are the dimensions of the 
    ...
    ```
    **The years from 1940-2002 are included in this dataset.  The data frame is 63 rows by 3 columns.  The column names are `year`, `boys`, and `girls`.**

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?  
    **The counts in the `present` data frame are far greater than those in Arbuthnot's data.  The first data points in this dataset represent roughly 1.1-1.2 million births of boys and girls; the Arbutnot's data did not have any data points over 8000, as indicated by the scales on the graphs in Exercise 2 above.**
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    ```
    **Arbuthnot's observation not appears valid, as a ratio of 1.00 would represent an even number of boys and girls being born in the U.S.**
...
-   In what year did we see the most total number of births in the U.S.? You can
    ...
    ```
    **The most total births occured in 1961.  This is supported by the plot below.**
    ```{r oyo-4-plot, echo = FALSE}
    plot(present$year, present$boys+present$girls, type = "l")
    ```
