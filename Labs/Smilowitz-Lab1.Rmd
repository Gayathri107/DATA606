...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    str(cdc)
    ```
    **There are 20,000 cases in this dataset (the number of rows in the data frame).  The variable types are as follows:**
    - `genhlth`: **categorical**  
    - `exerany`: **boolean**
    - `healhplan` : **boolean**
    - `smoke100`: **boolean**
...
2.  Create a numerical summary for `height` and `age`, and compute the 
    ...
    ```
    **There are 9569 males in the sample.  Roughly 23.3% of the sample report as being in excellent health.**
...
3.  What does the mosaic plot reveal about smoking habits and gender?  
    **This plot reveals that a higher percentage of men have smoked at least 100 cigarettes than have women.  This is apparent because among the `m` column, the `1` bar is larger than the `1` bar in the `f` column.**
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    **This plot shows that those with worse overall health generally have higher BMIs, and that the IQR of those with worse overall health is far larger than that of those with better overall health.**  
    ```{r boxplot, echo = FALSE}
    boxplot(bmi ~ cdc$gender, horizontal = TRUE)
    ```
      
    **This boxplot of BMI against gender indicates that men have a higher median BMI, but females have a much larger distribution of BMIs.**
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    **The two variables seem to be loosely positively correlated.  The positive correlation is sensible, as those with higher weight may have higher desired weights.  The loose correlation also makes sense, as there is narrower range of desired weights (most between 100-250 pounds), but a wider range of weights (around 100-400 pounds).  There are also two extemely high desired weights over 600 pounds.**
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    **`wdiff` is a continuous variable.  Positive values of this variable indicate that the person is above their desired weight; negative values mean the person is below their desired weight; values of 0 mean the person is precisely at their desired weight.**
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    ```{r wdiff-dist, echo=FALSE}
    par(mfrow = c(2,1))
    hist(cdc$wdiff, breaks = 100, freq = FALSE)
    boxplot(cdc$wdiff, horizontal = TRUE)
    par(mfrow = c(1,1))
    ```
      
    **`wdiff` is distributed with a general left-skewness, as seen in the histogram above.  However, if the two extreme negative outliers are ignored, the distribution is skewed right.  The distribution is centered around a value slightly above 0, with the first quartile at 0 and a third quartile around 25 pounds.  The spread is fairly narrow, with most values falling between roughly -50 and 100.  This tells us that most people are heavier than their ideal weightto varying degrees.** 
    
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    ```{r wdiff-gender}
    summary(subset(cdc, gender == 'm', wdiff))
    summary(subset(cdc, gender == 'f', wdiff))
    boxplot(cdc$wdiff ~ cdc$gender, subset = cdc$wdiff > -200)
    ```
      
    **From this data, and the boxplot (which ignores the extreme outliers below -200), it appears that women tend to view themselves as more overweight than men do, and men are more likely to view themselves as underweight (although most do still view themselves as overweight).**
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    z_neg_one <- mu - sigma
    z_pos_one <- mu + sigma
    one_sigma <- subset(cdc, weight >= z_neg_one & weight <= z_pos_one)
    max(dim(one_sigma))/20000
    ```
    **70.76% of the weights are within one standard deviation (40.08 pounds) within the mean of 169.68 pounds.**
