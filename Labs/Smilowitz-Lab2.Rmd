...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    **A streak length of 1 means one hit between two misses.  A steak length of 0 represents consecutive misses.**
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **Kobe's typical streak lengths were either zero (consecutive misses) or 1 (hits bookended by misses).  His longest streak was 4 consecutive made shots.**
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    ```
    **20 flips of the unfair coin came up heads**
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    ```{r independent-streak}
    ind_streak <- calc_streak(sim_basket)
    barplot(table(ind_streak))
    ```
      
    **The independent shooter has a typical streak of 0 shots, with 1-shot streaks the next-most common.  The longest streak in 133 baskets is a 5 shots.**
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    **I would expect additional simulations of the independent shooter to have a similar, but not identical, streak distribution.  The most common streak of 0 would likely remain since a given shot has a 55% probability of being a miss.  1-shot streaks will remain the most common, as after a hit, the next shot is most likely to be a miss.  Streaks of higher numbers will likely have the same approximate distribution, as they have equal probability - for an $n$ shot streak, the probability of $n$ consecutive shots going in is simply $P(n) = (0.45)^{n}$.**
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    **Kobe Bryan't distribution is more heavily weighted toward shorter shooting streaks (0 and 1), with a lower total range.  This is counter to the hot hand model, where we would expect a distribution with higher probabilities for longer streaks and a wider range.  If anything, this data is more in support of a cold hand model, as Kobe's performance is seemingly worse than an independent shooter.**
