...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    ```{r height-hist, echo=FALSE}
    # plot both histograms in the same image on the same x-axis
    par(mfrow = c(2,1))
    lbound <- round(min(bdims$hgt) - 10, -1)
    ubound <- round(max(bdims$hgt) + 10, -1)
    bins <- (ubound - lbound)/10
    hist(mdims$hgt, freq = FALSE, main = "Male Heights",   xlab = "", xlim = c(lbound, ubound), breaks = bins)
    hist(fdims$hgt, freq = FALSE, main = "Female Heights", xlab = "", xlim = c(lbound, ubound), breaks = bins)
    par(mfrow = c(1,1))
    ```
    
    **The distribution of male heights seems to be more normally distributed, with a wider spread than the distribution of female heights (which have a slight right-skewness.  Male heights also tend to be greater than female heights.**
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    **Based on the plot generated above, the data appear to approximate a normal distribution, though the distribtion has some asymmetry to it.**
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    ```{r qq-sim-norm, echo=FALSE}
    qqnorm(sim_norm)
    qqline(sim_norm)
    ```
      
    **The points do not precisely follow the line, though they do follow the line more closely than the real data, especially in the middle of the range (theoretical quantiles -1 to 1).**
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    **The `fdims$hgt` normal probability plot looks similar to the 8 simulated plots.  The similarity suggests that the female heights are nearly normal.**

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    ```{r weight, fig.height = 7}
    qqnormsim(fdims$wgt)
    ```
    
    **The normal probability plot for female weight differs significantly from the plots for the simulated data, particularly at high quantiles.  This suggests that female weights do not come from a normal distribution.**
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    **What is the probability that a woman is less than 5 feet (152 cm) tall?**
    ```{r under-5-ft}
    pnorm(q = 152, mean = fhgtmean, sd = fhgtsd)
    sum(fdims$hgt < 152) / length(fdims$hgt)
    ```
    
    **What is the probability that a woman weighs over 200 pounds (91 kg)?**
    
    ```{r over-200-lbs}
    1 - pnorm(q = 91, mean = mean(fdims$wgt), sd = sd(fdims$wgt))
    sum(fdims$wgt > 91) / length(fdims$wgt)
    ```
    
    **Height had a closer agreement between the two methods than weight (where the two were on different orders of magnitude).  This is a sensible result given the earlier observation that the distribution of heights more closely resembles a normal distribution than the distribution of weights.**
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter **B.  This can be observed by the long left tail in the histogram and associated low values in plot B.**

    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter **C.  This histogram most closely represents a normal distribution, and plot C reflects a near-normal distribution.**
    
    **c.** The histogram for general age (`age`) belongs to normal probability 
    plot letter **D, as evidenced by the strong right-skewness.**

    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter **A, as evidenced by the range of sample quantiles.**

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?
    
    **The variables associated with plots C and D, `elb.di` and `age` both have their highest frequency observations at z-scores separated from zero - this may be causing the stepwise pattern in the normal probability plots.**
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    **The higher values of sample quantiles at high theoretical quantiles suggests that distribution will be right-skewed.**
    
    ```{r knee-hist}
    hist(fdims$kne.di, breaks = 20)
    ```
    
    **The histogram confirms that the distribution is right-skewed.**
