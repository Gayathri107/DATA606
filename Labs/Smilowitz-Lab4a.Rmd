...
1.  Describe this population distribution.  
    **The distribution is right-skewed, ranging from 334 to 5642 with an IQR of roughly 617.**
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    ```{r hist-samp1, echo=FALSE}
    hist(samp1)
    ```
    
    **The sample distribution is right-skewed, ranging between `r min(samp1)` and `r max(samp1)`.  The overall shape of the distribution is similar to that of the population, but the sample has a narrower range and lesser degree of skewness.**
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    mean(samp2)
    ```
    **The mean of `samp2` is `r ifelse(mean(samp2) > mean(samp1), "greater ", "less ")` than the mean of `samp1`, but is quite close given the range of the population.  The sample of 1,000 would provide the more accurate estimate of the population mean, as it represents a larger portion of the population.** 
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?  
    **`sample_means50` contains 5,000 elements -- the mean of each of the 5,000 samples generated.  The sampling distribution appears approximately normal, centered around 1,500, which is the integer value of the population mean.  If 50,000 samples were collected, I would not expect the distribution to change greatly, but it might more precisely resemble a normal distribution.**
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    **`sample_means_small` contains 100 elements, where each element represents the mean of a sample of 50 observations from `area`.**
...
6.  When the sample size is larger, what happens to the center?  What about the spread?
    ...
    **The center of the sampling distribution is not significantly affected by the sample size -- each of the distributions is centered around the population mean.  However, recreating the above graphic higher level of granularity on the histograms reveals that the mean is slightly more accurately centered around the population mean with increasing sample size.  The spread decreases as sample size increases, with the values more closely centered around the mean with increasing sample size.**
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    mean(samp_price)
    ```
    **From this sample, the best estimate of the population mean is `r as.character(round(mean(samp_price),1))`.**
...
-   Since you have access to the population, simulate the sampling 
    ...
    **The sampling distribution appears approximately normal, centered around 180,000, with a range between `r as.character(round(min(sample_means50)))` and `r as.character(round(max(sample_means50)))`.  Based on the center of the sampling distribution, I would estimate the mean price of the population to be around 180,000.**
    
    ```{r price-mean}
    mean(price)
    ```
    
    **The actual mean price of the population is `r as.character(round(mean(price), 1))`.**
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    **The sampling distribution appears approximately normal, with a center slightly above 180,000 and a range between `r as.character(round(min(sample_means150)))` and `r as.character(round(max(sample_means150)))`.  This sampling distribution has a narrower spread than the distribution for a sample size of 50.  Based on the center of the sampling distribution, I would estimate the mean price of the population to be slightly more than 180,000.** 
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    **The distribution with a sample size of 150 has a smaller spread than that of sample size 50.  In order to arrive at more accurate estimates, a distribution with smaller spread is preferable --- this reduces the variability between sample means, leading to a point estimate that is likely to be closer to the population mean.**
