...
"typical" to mean.
    ...
    ```{r plot-sample}
    hist(samp, breaks = 15)
    boxplot(samp, horizontal = TRUE, add = TRUE, at = 10)
    ```
    
    **The distribution is right-skewed and centered around 1500.  The "typical" size is round 1500 square feet, where typical is the median value.**
    

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
    
    **I would not expect another student's distribution to be identical, as the probability of selecting an identical sample of 60 homes from the `r length(population)` homes is infinitesimally small.  I would expect it to have a similar mean, but there would likely be a good deal of variability in the sample mean, as 60 homes represents a small portion of the total population.**
...
met for this to be true?
    ...
    **In order for the standard error to be equal to $s / \sqrt{n}$, the $n$ observations must be independent, and the sample standard deviation $s$ must be drawn from a sample of at least 30 and the population must not be strongly skewed.**

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

    **95% confidence means that 95% of sample means $\pm$ the confidence interval will contain the population mean.**
...
Ames?
    ...
    **Yes, the population mean `r round(mean(population),2)` is contained in the confidence interval `r round(c(lower, upper),2)`.**
...
capture the true population mean? Why?
    ...
    **Because we are working with a 95% confidence interval, it is expected that 95% of students' confidence intervals will contain the population mean, per the definition of a 95% confidence interval outlined in the answer to Excercise 4 above.**
...
-   Using the following function (which was downloaded with the data set), 
    ...
    ```{r ci-include-pop, echo=FALSE}
    num_include_pop <- sum(lower_vector < mean(population) & upper_vector > mean(population))
    prop_include_pop <- round(num_include_pop / 50 * 100)
    ```
    
    **`r num_include_pop` of the 50 intervals, or `r prop_include_pop`% of intervals include the population mean.  The proportion does not exactly match the confidence level of 95%.  This is likely due to the fairly small sample size of only 60 homes - about 2% of the population.  Also, because there are 50 samples, it is not possible for precisely 95% of intervals to contain the population mean -- each interval represents 2% of the intervals calculated, so the closest values possible are 94% and 96%.**

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
    **For a confidence level of $X \%$, we are interested in the $z$ value that has $100-X \%$ of observations outside of that many standard deviations from the mean.  Since we are interested in the distribution function of $\frac{100-X}{2} \%$.  So for a 99% confidence interval, the critical value is 2.58.**
    
    ```{r critical-value}
    round(qnorm((1 - 0.99)/2, lower.tail = FALSE), 2)
    ```
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    ```{r ci-include-pop-99, echo=FALSE}
    num_include_pop <- sum(lower_vector < mean(population) & upper_vector > mean(population))
    prop_include_pop <- round(num_include_pop / 50 * 100)
    vs_ci <- ifelse(prop_include_pop < 99, "greater", "less")
    ```
    
    **`r num_include_pop` of the 50 intervals, or `r prop_include_pop`% of intervals include the population mean.  This is `r vs_ci` than the selected confidence level of 99% --- as above, each interval represents 2% of the intervals calculated, so it is not possible for 99% of the confidence intervals to include the population mean.**
