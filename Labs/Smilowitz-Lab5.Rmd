...
1.  What are the cases in this data set? How many cases are there in our sample?
    ...
    **The cases are births of children in North Carolina.  There are `r nrow(nc)` cases in the sample.**
...
highlight about the relationship between these two variables?
    ...
    **This boxplot shows that babies born to mothers who are smokers have a lower median weight than those who are born to nonsmokers.**
...
`length`.
    ...
    ```{r inference-conditions, echo=FALSE}
    by(nc$weight, nc$habit, length)
    ```
    
    **The conditions for inference are met -- the sample sizes are both large; the sample is randomly selected (per the description of the dataset) and represents less than 10% of babies born in a year; and the there is nothing to suggest that the weight of babies is heavily skewed.**
...
non-smoking mothers.
    ...
    inference(y = nc$weight, x = nc$habit, est = "mean", type = "ci", null = 0, alternative = "twosided", method = "theoretical", inf_plot = FALSE)
    ```
...
omit the `x` variable from the function.
    ...
    ```{r weeks-ci-95}
    inference(y = nc$weeks, est = "mean", type = "ci", method = "theoretical", inf_plot = FALSE)
    ```
    
    **From this confidence interval, we are 95% sure that the average length of all pregnancies lies between 38.1528 and 38.5165 weeks.**
...
to the function: `conflevel = 0.90`.
    ...
    ```{r weeks-ci-90}
    inference(y = nc$weeks, est = "mean", type = "ci", method = "theoretical", conflevel = 0.90, inf_plot = FALSE)
    ```
...
younger mothers is different than the average weight gained by mature mothers.
    ...
    **The difference in mean weight gained by younger moms and mature moms is not statistically significant at the 5% signfificance level.**
...
mothers. Use a method of your choice, and explain how your method works.
    ...
    **Using the `by` command splits `mage` variable into `mature` groups, and returns the range of mother ages for each group.  The cutoff age is 35 years old -- moths at or above this age are considered mature, and mothers younger than this age are considered younger.**
...
plain language.
    ...
    **Is there evidence that newborn male babies weigh more than newborn female babies?**
    ...
    **There is convincing evidence that the average weight of male babies is higher than the average weight of female babies.  The null hypothesis that babies of both genders have the same average weight would be rejected at any non-zero significance level.**
