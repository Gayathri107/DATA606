...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    **The reported findings appear to be sample statistics, as they correspond to the percentages of those polled.**
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    **In order to generalize the findings, the sample must be randomly selected to ensure independence of the sample.  While not explicitly stated, this seems like a reasonable assumption.**
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    **Each row of Table 6 corresponds to a country.  Each row of `atheism` corresponds to a person polled.**
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    **The proportion of atheist responses in `us12` (4.99%) matches that in Table 6 (5%).**
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    **In order to construct a confidence interval, the sample must be independent, and there must be at least 10 success and 10 failures expected.  The sample size of 1002 corresponds to less than 10% of the US population and appears to be randomly selected, so it meets conditions for independence.  If a success is considered an atheist response, then both $np$ and $n(1-p) where $p = 0.0499$ are $\geq 10$.  Based on this, I am confident all conditions are met.**
...
6.  Based on the R output, what is the margin of error for the estimate of the 
    ...
    **The margin of error is half of the range of the confidence interval, or $\pm `r round(100 * (0.0634 - 0.0364)/2, 2)` \%$.**
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    inference(canada12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    inference(brazil12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    ```
    
    **The 95% confidence interval for Canada is $(7.21\%, 10.75\%)$.  The estimate is $8.98 \%$, and the margin of error is $\pm `r round(100 * (0.1075 - 0.0721)/2, 2)` \%$.**
    
    **The 95% confidence interval for Brazil is $(0.56\%, 1.43\%)$.  The estimate is $1.00 \%$, and the margin of error is $\pm `r round(100 * (0.0056 - 0.0143)/2, 2)` \%$.**
...
8.  Describe the relationship between `p` and `me`.
    ...
    **For a fixed sample size `n`, the margin of error `me` has an inverse parabolic relationship to the population proportion `p`.  `me` increases at a slowing rate as `p` increases from 0, peaks at `p = 0.5`, and decreases at increasing rate as `p` moves towards 1.**
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    **The sampling distribution is centered at `r round(mean(p_hats), 2)` and appears normal, with a standard deviation of `r round(sd(p_hats), 3)`.**
...
10. Repeat the above simulation three more times but with modified sample
    ...
    ```{r sims, fig.width=10, fig.height=6, echo=FALSE}
        par(mfrow = c(2,2))
        ps <- c(0.1, 0.1, 0.02, 0.02)
        ns <- c(1040, 400, 1040, 400)
    ...
    **Based on these plots, increasing $n$ and decreasing $p$ both lead to a narrowing of the distribution of $\hat{p}$.**
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    **Based on the differences in the plots above (the top left and bottom right plots), I do not think that it is sensible to proceed with the generalized margins of error.  The difference between sample sizes and sample proportions reflect that the point estimates provided will have varying margins of error.**
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    inference(spain05$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    inference(spain12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    ```
    
    **There is not convincing evidence that Spain saw a change in its atheism index -- the 95% confidence intervals in the two years share significant overlap.**
    ...
    **b.** Is there convincing evidence that the United States has seen a
    ...
    inference(us05$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    inference(us12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist", eda_plot = FALSE)
    ```
    
    **There is convincing evidence that the atheism index in the US has changed between 2005 and 2012 -- the 95% confidence interval for the population proportion of $(0.0038, 0.0161)$ in 2005 and the corresponding interval in 2012 of $(0.0364, 0.0634)$ do not overlap.**
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    **The probability of a Type I error is equal to the significance level $\alpha$, in this case 0.05.  Table 4 includes 39 countries, so the expected number of Type I errors would be 1.95.**
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    **Rearranging this equation to solve for $n$ for a given $p$ and $ME$,**
    
    $n = (\frac{1.96}{ME})^{2} p (1-p)$
    
    **The plot generated for Exercise 8 above shows that margin of error is maximized at $p = 0.5$.  Using this value in the equation above to guarantee a maximum margin of error of 1%, $n = 9604$.**
