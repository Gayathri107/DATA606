...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    **A scatter plot is likely the most useful plot to investigate the relationship between two numerical variables.**
    
    ```{r scatter-at-bats, echo=FALSE}
    plot(x = mlb11$at_bats, y = mlb11$runs, type = "p", xlab = "at-bats", ylab = "runs", main = "Runs vs. At-Bats")
    ```
    
    **The relationship appears linear, but appears to have a low correlation.  I would be comfortable using a linear model, but would be cautious to monitor the R-square value.**
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    **The relationship appears linear and positive, and appears somewhat weak.  The point a roughly (5520, 870) seems to be very far off from this relationship.**
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    **In running the `plot_ss` function ten times, the lowest Sum of Squares I was able to achieve was 124827.5**
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    ...
    $\hat{y} = 415.2389 + 1.8345 \times homeruns$
    
    **The positive value of the slope indicates that there is a positive relationship between homeruns and runs scored.**
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    **The predicted value can be calculated using the coefficients from the `m1` linear model.  There is no value of exactly 5,578 at-bats, so the closest value is found and used.**
    ```{r residual}
    library(dplyr)
    (prediction <- m1$coefficients[1] + m1$coefficients[2] * 5578)
    actual <- mlb11 %>% filter(at_bats > 5575, at_bats < 5580) %>% select(runs)
    (residual <- prediction - actual)
    ```
    **The manager would predict that a team with 5,578 at-bats would score about 728 runs.  This is an overestimate of about 15 runs**
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    **There does not appear to be any pattern in the residual plot, although there is a large spread on the residuals.  This indicates that the relationship is likely linear, but weak.**
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    **The histogram appears roughly normal (though may be right-skewed), and the normal probability plot does not differ greatly from the normal line, so it appears that the normality condition of residuals appears to hold.**
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    **The scatterplot in (1) appears to be have roughly constant variablity.**
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    **Using `hits` as the predictive variable**
    ```{r hits-runs, echo=FALSE}
    plot(x = mlb11$hits, y = mlb11$runs, xlab = "hits", ylab = "runs", main = "Runs vs. Hits")
    ```
    
    **At a glance, there appears to be a positive linear relationship between the two variables.**
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    **Based on the R-squared values, `hits` has a stronger relationship with `runs` than `at_bats` does.  The value of 0.6419 indicates that about 64% of variation in `runs` can be explained by variation in `hits`, much better than the 37% predicted by variablity in `at_bats`.  It is also a better predictor of `runs` -- the residual standard error of 50.23 for this model is lower than the 66.47 for the model using `at_bats` as a predictor.**
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    **Among the traditional variables, the best predictor is batting average (`bat_avg`) -- this relationship has an R-squared value of 0.6561.**
    
    ```{r plot-bat-avg, echo=FALSE}
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    **The new variables are much better predictors of team success (as indicated by runs scored) -- each R-squared value is greater than 0.8, while the best traditional predictor had an R-squared below 0.7.**
    
    ```{r plot-new, echo=FALSE, fig.height=10}
    par(mfrow = c(3,1))
    ...
    **The best predictor of `runs` is `new_obs`, or on-base plus slugging.  This makes sense, as it is a combination of on-base percentage (an indicator of how often a batter gets on base) and slugging percentage (a measure of how many bases, on average, the batter advances for each at-bat).**
...
-   Check the model diagnostics for the regression model with the variable you 
    ...
    **The scatterplot of residuals shows no pattern and appears roughly equal, so the conditions for linearity and contsant variability are met.  The histogram appears roughly normal, and the normal probability plot appears to indicate no deviations from normality, so the normality condition of residuals can be accepted.**
